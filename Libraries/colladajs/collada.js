...
// Modified by JK as follows:
//  Added 'glMatrix' prefixes (so that glMatrix function names aren't global in scope)
...
global.Collada = _collada = {
	...
	init: function (config)
	{
		...
		//init glMatrix
		temp_mat4 = glMatrix.mat4.create();
		temp_vec2 = glMatrix.vec3.create();
		temp_vec3 = glMatrix.vec3.create();
		temp_vec4 = glMatrix.vec3.create();
		temp_quat = glMatrix.quat.create();
	...
	readTransform: function(xmlnode, level, flip)
	{
		...
		for(var i = 0; i < xmlnode.childNodes.length; i++)
		{
			...
			if(xml.localName == "translate")
			{
				...
				glMatrix.mat4.translate( matrix, matrix, values );
				continue;
			}
			...
			if(xml.localName == "rotate")
			{
				...
				if(values.length == 4) //x,y,z, angle
				{
					...
					if(values[3] != 0.0)
					{
						glMatrix.quat.setAxisAngle( tmpq, values.subarray(0,3), values[3] * DEG2RAD);
						glMatrix.mat4.fromQuat( temp, tmpq );        
						glMatrix.mat4.multiply(matrix, matrix, temp);
					}
				}
				continue;
			}
			...
			if(xml.localName == "scale")
			{
				...
				glMatrix.mat4.scale( matrix, matrix, values );
			}
		}
	...
	readTransform2: function(xmlnode, level, flip)
	{
		...
		var tmpq      = glMatrix.quat.create();         
		var translate = glMatrix.vec3.create();         
		...
		for(var i = 0; i < xmlnode.childNodes.length; i++)
		{
			...
			if(xml.localName == "rotate")
			{
				...
				if(values.length == 4) //x,y,z, angle
				{
					...
					{
						glMatrix.quat.setAxisAngle( tmpq, values.subarray(0,3), values[3] * DEG2RAD);
						glMatrix.quat.multiply(rotation,rotation,tmpq);
					}
				}
				continue;
			}
		...
		glMatrix.mat4.fromQuat( tmpmatrix , rotation );
		//mat4.rotateX(tmpmatrix, tmpmatrix, Math.PI * 0.5);
		glMatrix.mat4.multiply( matrix, matrix, tmpmatrix );
	...
	readSkinController: function( xmlskin, flip, scene )
	{
		...
		if(xmlbindmatrix)
		{
			...
			bind_matrix = glMatrix.glMatrix.mat4.create(); //identity
	...
	transformMatrix: function(matrix, first_level, inverted)
	{
		...
		if(first_level){
			...
			glMatrix.vec4.scale(temp,temp,-1);
		}
		else 
		{
